...
// Return the correct icon for a vehicle
const getVehicleIcon = (type: VehicleType) => {
    if (type === "car") return carIcon;
    if (type === "ev") return evIcon;
...
export default function Page() {
    ...
    const routeCoords: [number, number][] =
        destinationCoords
            ? [userCoords, [(userCoords[0] + destinationCoords[0]) / 2, (userCoords[1] + destinationCoords[1]) / 2,], destinationCoords,] : [];
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-gray-900 p-6 md:p-10">
            ...
            <div className="relative h-[500px] w-full rounded-3xl overflow-hidden shadow-lg border border-gray-200">
                ...
                <MapContainer center={userCoords} zoom={14} className="h-full w-full" zoomControl={false}>

                    <TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                    <MapClickHandler onMapClick={handleMapClick} />
                    <Marker position={userCoords} icon={userIcon} >
                        <Popup>
                            <strong>  Your Location </strong> <p> {userAddress} </p>
                        </Popup>
                    </Marker>
                    ...
                    {destinationCoords && (
                        <Marker position={destinationCoords} icon={destinationIcon}>
                            <Popup>
                                <strong> Destination </strong> <p>  {destinationName} </p>
                            </Popup>
                        </Marker>
                    )}
                    ...
                    {routeCoords.length > 1 && (
                        <Polyline positions={routeCoords} pathOptions={{ color: "#2563eb", weight: 4, dashArray: "6, 6" }} />
                    )}

                </MapContainer>


                <div className="absolute top-4 left-4 z-150 bg-white/90 backdrop-blur rounded-2xl shadow-md p-1.5 flex gap-1">

                    <button onClick={() => setSelectedType("all")} className={` px-3 py-1.5 rounded-xl text-xs font-semibold ${selectedType === "all" ? "bg-black text-white" : "hover:bg-gray-100"}`}
                    >
                        All
                    </button>
                ...
                <div className="absolute bottom-4 left-4 right-4 z-10 max-w-md bg-red-50">
                    {selectedVehicle ? (
